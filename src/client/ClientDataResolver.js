  ...
  resolveFile(resource) {
    ...
    if (typeof resource === 'string') {
      if (/^https?:\/\//.test(resource)) {
        return fetch(resource).then(res => res.ok ? res.arrayBuffer().then(buffer => Buffer.from(buffer)) : Promise.reject(new Error(`Failed to fetch resource: ${res.status} ${res.statusText}`)));
      }
