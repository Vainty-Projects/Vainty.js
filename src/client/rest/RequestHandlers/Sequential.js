      ...
      item.request.gen(captchaKey, captchaToken).end(async (err, res) => {
        ...
        if (err) {
          if (err.status === 429) {
            ...
            if (res && res.body?.captcha_service) {}
            /*if (res && res.body?.code && res.body.code === 60003 && item.retries < 1){
              ...
              const mfaPost = await this.manager.client.api.mfa.finish.post({
                data: {
                  ...
                  mfa_type: mfaData.method[0].type,
                },
              });
              item.options.mfaToken = mfaPost.token;
              item.retries++;
            ...
            if (
              ...
              const captcha = await this.restManager.captchaService.solve(
                res.body,
                `${this.restManager.userAgentManager.userAgent}`,
              );
              item.retries++;
