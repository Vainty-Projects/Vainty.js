...
class VoiceConnection extends EventEmitter {
  ...
  startStreaming() {
    this.client.ws.send({
      op: Constants.OPCodes.STREAM_CREATE,
      d: {
        type: ['dm', 'group'].includes(this.channel.type) ? 'call' : 'guild',
        guild_id: this.channel.guild?.id || null,
        channel_id: this.channel.id,
  ...
  stopStreaming() {
    this.client.ws.send({
      op: Constants.OPCodes.STREAM_DELETE,
      d: {
   ...
   */
  get streamKey() {
    return `${['dm', 'group'].includes(this.channel.type) ? 'call' : `guild:${this.channel.guild.id}`}:${
      this.channel.id
    }:${this.client.user.id}`;
  }
